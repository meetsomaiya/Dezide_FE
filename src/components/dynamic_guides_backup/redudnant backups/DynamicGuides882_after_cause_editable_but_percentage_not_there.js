...
<tr className="sub-cause-row" key={subIndex}>
  ...
  <td>
    {editingField886?.index === subIndex && editingField886?.field === "CauseName" ? (
      <input
        ...
        onChange={(e) => handleSaveSubField886(subIndex, "CauseName", e.target.value)}
  ...
  <td>
    {editingField886?.index === subIndex && editingField886?.field === "ProbabilityPercentage" ? (
      <input
        type="text"
        value={causeDetail.ProbabilityPercentage || ""}
        onChange={(e) =>
          handleSaveSubField886(subIndex, "ProbabilityPercentage", e.target.value)
        }
        onBlur={() => exitSubEditMode886()}
        autoFocus
      />
    ) : (
      <span onClick={() => handleEditSubField886(subIndex, "ProbabilityPercentage")}>
