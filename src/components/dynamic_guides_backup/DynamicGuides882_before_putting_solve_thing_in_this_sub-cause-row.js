    ...
    {causesData.map((cause, index) => (
      <React.Fragment key={index}>
        ...
        >
          {/* Tick mark column */}
          <td>
            {/* Conditionally render the break icon based on checkbox state */}
            {!solveCheckboxes900[index] && <span>✓</span>}
          </td>
          ...
          expandedCauseData.map((causeDetail, subIndex) => (
            <React.Fragment key={`${index}-${subIndex}`}>
              <tr className="sub-cause-row" key={subIndex}>
                {/* Aligning the sub-cause rows under the right columns */}
                <td></td>
                <td>
                ...
                {/* Add any additional sub-cause specific columns if needed */}
              </tr>
