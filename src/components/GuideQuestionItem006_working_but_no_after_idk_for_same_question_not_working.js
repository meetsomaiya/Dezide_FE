...
const handleQuestionClick = (e, question) => {
  ...
  const handleOptionSelect = async (option) => {
    ...
    if (option === "Yes") {
      ...
      setTrackerCount(0); // Reset tracker count
      setLastIDontKnowCount(null); // Reset last "I don't know" tracker
      setLastAction(null); // Clear last action
      setCurrentActionName(""); // Clear current question
      return; // Exit as there's no need for further processing
    }
  
    if (option === "I don't know") {
      newTrackerCount = trackerCount + 1; // Increment tracker count
    ...
    // Fetch the next question using the updated tracker count
    const dataToSend = {
    ...
    try {
      ...
      if (data && data.actionName) {
        ...
        sendActionNameToApi(data.actionName); // Fetch explanation and image for the next action
      }
    } catch (error) {
