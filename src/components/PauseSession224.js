...
const PauseSession224 = () => {
  ...
  const handleResume = async () => {
    try {
      // Log the session ID that will be sent to the API
      console.log('Sending session ID to API:', inputSessionId);

      // Send the session ID entered in the input box to the API
      const response = await fetch('http://localhost:226/api/fetch_resume_data', {
        method: 'POST', // Sending data with POST method
        headers: {
          'Content-Type': 'application/json', // Set the request content type
        },
        body: JSON.stringify({ session_id: inputSessionId }), // Send the inputSessionId in the request body
      });
