...
const [menuPosition902, setMenuPosition902] = useState({ top: 0, left: 0 });
  ...
  const handleSubCauseSliderChange = (subIndex, value) => {
   ...
   const handleCreateTopCauseClick = () => {
    ...
    // Update causesData state with the new cause
    setCausesData(prevCauses => {
      const updatedCauses = [{ ...newCause }, ...prevCauses]; // Add new cause at the start of the array
    ...
    setShowOptionsBox1112(!showOptionsBox1112); // Toggle options box visibility
  
    // Call saveCauseData after the new cause has been added to causesData
    // Ensure the new cause is added before calling saveCauseData
    setTimeout(() => {
      saveCauseData(); // This ensures saveCauseData is called after state updates
    }, 0); // 0 delay to wait for state updates to complete
  };
