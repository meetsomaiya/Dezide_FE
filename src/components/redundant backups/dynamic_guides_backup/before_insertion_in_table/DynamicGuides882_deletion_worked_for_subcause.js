  ...
  // };
    ...
    const deleteRow = (type, identifier) => {
      if (type === 'cause') {
        ...
        const [subCauseIndex, nestedSubCauseIndex] = identifier.split('-').map(Number);
    
        // Extract the nested sub-cause key from expandedCauseData
        const nestedSubCauseKey = expandedCauseData[subCauseIndex]?.CauseName;
    
        if (nestedSubCauseKey && nestedSubCauseData[nestedSubCauseKey]) {
          const updatedNestedSubCauses = nestedSubCauseData[nestedSubCauseKey].filter(
            (_, index) => index !== nestedSubCauseIndex
          );
    
          // Update the nestedSubCauseData with the filtered data
          const updatedNestedSubCauseData = {
            ...nestedSubCauseData,
            [nestedSubCauseKey]: updatedNestedSubCauses,
          };
    
          setNestedSubCauseData(updatedNestedSubCauseData);
          console.log(
            `Deleted Nested SubCause at index: ${nestedSubCauseIndex} for key: ${nestedSubCauseKey}`
          );
        } else {
          console.error('Error: Nested SubCause Key not found or invalid data structure.');
        }
      }
    };
