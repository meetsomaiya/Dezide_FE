    ...
    const deleteRow = (type, identifier) => {
      if (type === 'cause') {
        ...
        if (causeToDelete) {
          ...
          // Send the data to API without modifying the causesData array
          const payload = {
            modalName,
            causeName,
            deletionFlag: true,
          ...
          fetch("http://localhost:226/api/deleted_top_cause", {
            // fetch("http://localhost:3001/api/deleted_top_cause", {
            method: "POST",
        ...
        if (Array.isArray(expandedCauseData) && expandedCauseData[subCauseIndex]) {
          ...
          // Send the data to API without modifying the expandedCauseData array
          const payload = {
            modalName,
            parentCauseName,
            subCauseName,
