...
//   // Define the new nested sub-cause with default values
//   const newNestedSubCause = {
//     eventName: "Untitled Nested SubCause", // Default event name
...
//   // Check if the key exists in nestedSubCauseData and update accordingly
//   setNestedSubCauseData((prevState) => {
//     const updatedState = {
//       ...prevState,
//       [key]: prevState[key]
//         ? [...prevState[key], newNestedSubCause] // Append new nested sub-cause
//         : [newNestedSubCause], // Initialize with the new nested sub-cause if key does not exist
//     };
//     return updatedState;
//   });
...
const addNewUntitledNestedSubCause = (causeIndex, subCauseIndex) => {
  ...
  // Update the nestedSubCauseData with a new nested sub-cause
  setNestedSubCauseData((prevState) => {
    const existingSubCauses = prevState[key] || [];

    // Determine the suffix for the new sub-cause
    const baseName = "Untitled Nested SubCause";
    let suffix = 1;

    // Check existing sub-cause names to determine the next available number
    while (
      existingSubCauses.some(
        (subCause) => subCause.eventName === `${baseName} ${suffix}`
      )
    ) {
      suffix++;
    }

    // Define the new nested sub-cause with the calculated name
    const newNestedSubCause = {
      eventName: suffix === 1 ? baseName : `${baseName} ${suffix}`, // Add suffix if needed
      probability: 0, // Default probability
    };

    // Update the state with the new nested sub-cause
    const updatedState = {
      ...prevState,
      [key]: [...existingSubCauses, newNestedSubCause], // Add the new sub-cause to the array
    };
