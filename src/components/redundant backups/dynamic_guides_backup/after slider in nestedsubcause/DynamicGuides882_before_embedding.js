...
onMouseEnter={() => {
  ...
  setHoveredCell1114(index); // Set hovered cell state for the icon
}}
onMouseLeave={() => {
  setHoveredCause993(null); // Clear hovered cause when mouse leaves
  setHoveredCell1114(null); // Clear hovered cell when mouse leaves
}}
...
>
  ...
  {editingField886?.index === subIndex && editingField886?.field === "CauseName" ? (
    <input
      type="text"
      value={editingField886.value || ""} // Controlled value, default to an empty string
      onChange={(e) => setEditingField886({ ...editingField886, value: e.target.value })} // Update value while typing
      onBlur={(e) => {
        // Save on blur
    ...
    <span onClick={() => setEditingField886({ index: subIndex, field: "CauseName", value: causeDetail.CauseName || "" })}>
      {causeDetail.CauseName || "Click to edit"} {/* Show placeholder if blank */}
    </span>
  )}
</td>
            ...
            <td
onMouseEnter={() => setHoveredCell1114(`${index}-${subIndex}`)}
