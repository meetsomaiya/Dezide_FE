...
const handleSaveNestedSubCauseField7773 = (key, nestedIndex, field, newValue) => {
  ...
  if (isChecked) {
    ...
    if (selectedAction) {
      // Prepare data for the API (even if progress is missing)
      const payload = {
        actionName: selectedAction.name,
        time: selectedAction.time,
        money: selectedAction.money,
        ...
        causeName,
        modalName,
        // Include progress if it exists, otherwise send null/undefined
        progress: selectedAction.progress || null, // Optional
      };
