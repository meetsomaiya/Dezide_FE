...
const [modalName, setModalName] = useState("");
  ...
  const handleSubCauseToggleAndFetch = async (key, subCauseName) => {
    ...
    // Fetch data only if expanding the sub-cause
    if (!expandedSubCause[key]) {
      try {
        const response = await fetch(
          `http://localhost:226/api/fetch_sub_cause_based_on_causee?CauseName=${encodeURIComponent(
            subCauseName
          )}`
        );
  ...
  {causesData.length > 0 ? (
    ...
    >
      ...
      {expandedSubCause[`${cause.name}-${causeDetail.CauseName}`] ? '-' : '+'}
              ...
              {expandedSubCause[`${cause.name}-${causeDetail.CauseName}`] &&
                causeDetail.SubCauses &&
                causeDetail.SubCauses.map((subCause, nestedIndex) => (
                  <tr
