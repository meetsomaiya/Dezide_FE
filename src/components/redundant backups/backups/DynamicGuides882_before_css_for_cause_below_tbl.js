        ...
        {causesData.length > 0 ? (
          causesData.map((cause, index) => (
            <React.Fragment key={index}>
              ...
              {/* Render the expanded data row ONLY if the current row's name is expanded */}
              {expandedCauseName === cause.name && expandedCauseData && (
                <tr>
                  <td colSpan="3" className="expanded-row">
                    {/* Render the fetched CauseName and ProbabilityPercentage */}
                    {expandedCauseData.map((causeDetail, index) => (
                      <div key={index}>
                        <strong>{causeDetail.CauseName}</strong> - 
                        {causeDetail.ProbabilityPercentage}%
                      </div>
                    ))}
                  </td>
                </tr>
