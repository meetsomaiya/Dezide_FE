...
const [modalName, setModalName] = useState("");
  ...
  const handleCreateTopCauseClick = () => {
    setCreateTopCauseBoxVisible((prevState) => !prevState);
  };
  
  const handleAddTopCause = () => {
    setTopCause({ name: topCauseText, probability: 0 }); // Default probability for the new cause
    setTopCauseText(''); // Reset the text box in the Create Top Cause box
    setCreateTopCauseBoxVisible(false);
  };
  
  const handleTopCauseTextChange = (e) => {
    setTopCauseText(e.target.value);
  };
  const tableData = [
