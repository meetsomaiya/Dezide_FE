...
const GuideQuestionItem = () => {
  ...
  const sendActionNameToApi = async (actionNameToSend) => {
    try {
      ...
      if (data) {
        setActionExplanation(data.explanation); // Set explanation text
        setActionImage(data.image); // Set base64 image data
      }
    } catch (error) {
    ...
    <div className="container006">
      <div className="left-section006">
        ...
        <div className="explanation-box006">
          <h3>Explanation</h3>
          <p>{actionExplanation || "Here is some dummy explanation text. For now, this is a placeholder text explaining the topic at hand."}</p>
          
          {/* Render the image if available */}
          {actionImage && (
            <div className="action-image-container">
              <img src={`data:image/png;base64,${actionImage}`} alt="Action Explanation" className="action-image" />
            </div>
          )}
        </div>
