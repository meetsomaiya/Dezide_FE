...
//     }
  ...
  const updateModalNameOnBackend = async (name) => {
    try {
      const response = await fetch('http://localhost:3001/update_modal_for_event', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ modalName: name }),
      });
  
      const data = await response.json();
      console.log('Data sent to backend:', { modalName: name });
...
<div
  ...
  onClick={() => {
    if (!isEditing882) {
      setPreviousName(modalName); // Store current name as previous name when editing starts
    ...
    <input
      ...
      onBlur={async () => {
        ...
        if (modalName !== previousName) {
          ...
          console.log("Sending to API:", payload); // <-- This will log the values
